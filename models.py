...
class User(db.Model):
    ...
    auto_prediction_strategies = db.Column(db.String(100), default='hot,cold,trend,hybrid,balanced,ml')  # 自动预测策略，多个策略用逗号分隔
...
class PaymentOrder(db.Model):
    ...
    channel = db.Column(db.String(32), nullable=False, default='alipay_f2f')
    purpose = db.Column(db.String(32), nullable=False, default='activation')
    subject = db.Column(db.String(128), nullable=False)
    amount = db.Column(db.Numeric(10, 2), nullable=False, default=0)
    validity_type = db.Column(db.String(20), nullable=False, default='month')
    status = db.Column(db.String(20), nullable=False, default='created', index=True)
    alipay_trade_no = db.Column(db.String(64), index=True)
    buyer_logon_id = db.Column(db.String(128))
    qr_code = db.Column(db.Text)
    issued_code = db.Column(db.String(64))
    raw_request_payload = db.Column(db.Text)
    raw_response_payload = db.Column(db.Text)
    raw_notify_payload = db.Column(db.Text)
    ...
    def validity_label(self):
        ...
        return labels.get(self.validity_type, self.validity_type or '')
