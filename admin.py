...
def _alipay_f2f_enabled():
    raw = str(SystemConfig.get_config('alipay_f2f_enabled', 'false')).strip().lower()
    return raw in {'true', '1', 'yes', 'on'}


def _wechat_native_enabled():
    raw = str(SystemConfig.get_config('wechat_native_enabled', 'false')).strip().lower()
    return raw in {'true', '1', 'yes', 'on'}


def _payment_enabled():
    return _alipay_f2f_enabled() or _wechat_native_enabled()
...
def payment_orders():
    ...
    orders = query.order_by(PaymentOrder.created_at.desc(), PaymentOrder.id.desc()).paginate(
        page=page,
        per_page=20,
        error_out=False,
    )
    user_ids = sorted({order.user_id for order in orders.items if order.user_id})
...
SYSTEM_CONFIG_DEFAULTS = {
    ...
    'alipay_public_key': '',
    'alipay_notify_url': '',
    'alipay_activation_product_name': '账号激活服务',
    'alipay_price_day': '1.00',
    'alipay_price_month': '9.90',
    'alipay_price_quarter': '26.00',
    'alipay_price_year': '88.00',
    'alipay_price_permanent': '199.00',
    'wechat_native_enabled': 'false',
    ...
    'wechat_notify_url': '',
    'wechat_activation_product_name': '账号激活服务',
}
