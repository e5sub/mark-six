...
def update_database():
    ...
    try:
        ...
        if not check_table_exists(cursor, 'payment_orders'):
            print("Creating payment_orders table...")
            cursor.execute('''
            ...
            print("payment_orders table already exists")

        if not check_index_exists(cursor, 'ix_payment_orders_status'):
            cursor.execute('''
                CREATE INDEX ix_payment_orders_status
                ON payment_orders (status)
            ''')

        if not check_index_exists(cursor, 'ix_payment_orders_alipay_trade_no'):
            cursor.execute('''
                CREATE INDEX ix_payment_orders_alipay_trade_no
